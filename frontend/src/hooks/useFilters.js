...
export function useFilters(labs, activeCategory) {
  ...
  const filteredLabs = useMemo(() => {
    ...
    return labs.filter((result) => {
      const inCategory = activeCategory === "overview" || result.category === activeCategory;
      ...
      const inSearch =
        !query ||
        result.displayName.toLowerCase().includes(query) ||
        result.panel.toLowerCase().includes(query) ||
